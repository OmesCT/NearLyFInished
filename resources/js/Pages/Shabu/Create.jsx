...
export default function Create() {
    ...
    const handleSubmit = (e) => {
        ...
        post("/reserve", {
            onSuccess: (page) => {
                console.log(page.props); // ตรวจสอบข้อมูลที่ส่งกลับมา
                if (page.props.booking && page.props.booking.id) {
                    window.location.href = `/show/${page.props.booking.id}`; // เปลี่ยนเส้นทางไปหน้า show พร้อมกับ ID ของการจอง
                } else {
                    console.error("Booking ID is missing in the response.");
                }
            },
            onError: (errors) => {
                console.error(errors); // แสดงข้อผิดพลาดในคอนโซล
            }
        });
    };
