...
export default function Reserve() {
    const { tables, reservations } = usePage().props;
            ...
            <div className="mt-10 flex justify-center">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 p-8 w-full max-w-4xl">
                    {tables && tables.length > 0 ? (
                        tables.map((table) => {
                            const reservation = reservations ? reservations.find(res => res.table_id === table.id) : null;
                            return (
                                <Card
                                    key={table.id}
                                    ...
                                    <CardContent className="p-3 flex items-center justify-between gap-4">
                                        ...
                                        <div className="flex-1 text-left">
                                            ...
                                            <p className="text-sm mt-1">
                                                {table.available ? "ว่าง" : `จองโดย ${reservation && reservation.user ? reservation.user.name : "ไม่ระบุ"}`}
                                            </p>
                                        </div>
